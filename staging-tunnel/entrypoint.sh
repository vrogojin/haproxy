...
# Install developer keys (mounted read-only at /auth/authorized_keys).
if [ -f /auth/authorized_keys ]; then
  install -o tunnel -g tunnel -m 600 /auth/authorized_keys /home/tunnel/.ssh/authorized_keys
  echo "[staging-tunnel] installed $(grep -c . /home/tunnel/.ssh/authorized_keys 2>/dev/null || echo 0) authorized key line(s)"
else
  echo "[staging-tunnel] WARN: no /auth/authorized_keys mounted — no one can tunnel" >&2
fi
